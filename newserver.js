...
app.post('/search', (req, res) => {
 ...
 //------------------------------------------------------------------------------------------------------------ 
  ...
  //------------------------------------------------------------------------------------------------------------ 
    const query = `
      SELECT usrid, planning, watching, completed, on_hold, dropped
      FROM user_watch_status
      WHERE usrid = ? AND anime_id IN (
        SELECT anime_id
        FROM mytable
        WHERE anime_id IN (?)
      )
    ...
    connection.query(query, [userId, animeIds], (err, results) => {
      if (err) {
        console.error('Error querying database:', err.message);
        return res.status(500).json({ error: 'Internal Server Error' });
      }
  
      const libraryStatus = results.map(result =>
        result &&
        Object.values(result).some(fieldValue =>
          fieldValue && animeIds.includes(fieldValue.toString())
        )
      );
...
app.post('/addToLibrary', (req, res) => {
  ...
  //------------------------------------------------------------------------------------------------------------ 
    ...
    const params = [UserId, animeIds.join(','), UserId, animeIds.join(',')];
